...
//RedemptionUO Start
		...
		public static void FactionBroadcast_OnCommand( CommandEventArgs e )
        {
              Mobile from = e.Mobile;

              PlayerState pl = PlayerState.Find( from );
			string message = String.Format( "[Faction][{0}]: {1}", from.Name, e.ArgString );
            ...
            for ( int i = 0; i < factions.Count; ++i )
            {
                ...
                if( playerStateList.Contains( pl ) )
                {
                    for( int j = 0; j < playerStateList.Count; ++j )
                        (playerStateList[j].Mobile).SendMessage( 0x35, message );
                   // If we found the faction the player is in, exit the function
                   return;

                }
                else if ( i != factions.Count - 1 ) // if they're not in this faction, and this is not the last faction in the list, go to the next faction
                  continue;

                // we can only get here if we checked all 4 factions and they're not in it
