...
namespace VitaNex.Modules.PowerHour
{
	public static partial class PowerHour
	{
			...
			if (!state.CheckState(m, true))
			{
				if (CMOptions.UseTimeBoosts && !state.CooldownEnded && m is PlayerMobile)
				{
					var ui = new TimeBoostsUI(m, null, TimeBoosts.TimeBoosts.EnsureProfile((PlayerMobile)m))
					{
						...
						GetTime = () => state.Cooldown,
						SetTime = t => state.Cooldown = t
					};
		...
		private static bool OnSkillCheckTarget(Mobile m, SkillName skill, object target, double min, double max)
		{
			...
			{
				SkillGainCheck(m, skill, min, max);
		...
		public static bool SkillGainCheck(Mobile m, SkillName skill, double min, double max)
		{
			if (HasPowerHour(m) && CMOptions.SkillGainChance > 0 && Utility.RandomDouble() < CMOptions.SkillGainChance)
			{
				return m.CheckSkill(skill, min, max);
			}

			return false;
		}

		public static bool SkillGainCheck(Mobile m, SkillName skill, double chance)
		{
			if (HasPowerHour(m) && CMOptions.SkillGainChance > 0 && Utility.RandomDouble() < CMOptions.SkillGainChance)
			{
				return m.CheckSkill(skill, chance);
			}
