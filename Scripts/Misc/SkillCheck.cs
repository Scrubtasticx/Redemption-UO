	...
	public class SkillCheck
	{
		...
		public static void Gain( Mobile from, Skill skill )
		{
			...
			if ( skill.Lock == SkillLock.Up )
			{
				...
				if ( from.StrLock == StatLockType.Up && (info.StrGain / 5.0) > Utility.RandomDouble() )
					GainStat( from, Stat.Str );
				else if ( from.DexLock == StatLockType.Up && (info.DexGain / 5.0) > Utility.RandomDouble() )
					GainStat( from, Stat.Dex );
				else if ( from.IntLock == StatLockType.Up && (info.IntGain / 5.0) > Utility.RandomDouble() )
